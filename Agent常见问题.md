...
- 离线批处理：
  - 场景：大数据摘要总结、评论标签
  - 特点：高吞吐、延迟不敏感、Padding（填充）问题
  - 技术栈：可用vLLM（Offline Mode）、Ray Data等
- 在线流式处理
  - 场景：问答助手、Coding助手  、实时翻译等
  - 特点：低延迟（首字延迟）、逐字生成
  - 协议：SSE、gPRC Strem
- 连续批处理：
  - vLLM等框架核心调度机制
  - 基于迭代（Iteration/Token）级别做批量处理，与传统请求级别区分避免饿死问题
  - 保证GPU始终满负荷跑，实现高吞吐、低延迟
## 2. 数据处理
- 批处理
  - 准备预训练/微调数据，即Spark/MapReduce任务
  - RAG全量索引（T+1更新），保障数据一致性，无法获取实时数据
- 流处理
  - RAG实时索引：比如基于Flink监听数据变更后立即完成向量化
  - 用户反馈流：通过流式管道（Kafka）送入数据库，用于RLHF（强化学习）或在线评估监控
## 3. 架构设计
- 传统Http请求：等待服务端返回完整响应，适用于后台任务
- 流式传输：
  - SSE：LLM事实标砖，单向流，基于Http，前端处理简单
  - WebSocket：双工通信，大模型实时语音对话等场景

# 四、LLM应用架构
## 1. 基础架构
- 前端：流式数据接收，基于SSE协议
- 网关层：鉴权、Token限流、安全护栏
- 应用层：Prompt组装、RAG、Tool模块、Memory模块
- 模型层：vLLM集群，并发推理
- 运营平台：查看Trace日志，标注bad case，迭代prompt
## 2.高并发架构
1. 核心矛盾：昂贵的显存/算力资源、高并发/低延迟的用户请求
2. 解决方案：
   - LLM推理引擎：
     - PagedAttention：减少显存碎片化有，提高显存利用率
     - KV Cache量化：降低精度和显存占用，提高上下文长度和并发
     - 连续批处理：提高GPU利用率，面向迭代批处理
     - 共享前缀：多个请求共享相同System Prompt，只存一份KV Cache
   - 系统架构：
     - 模型网关：简单模型->小模型、复杂问题->大模型
     - 业务层：异步处理&消息队列，长文本生成、文档分析等走kafka异步处理，前端SSE推送结果或者轮询
     - RAG数据层：存算分离&冷热分层，向量库只存索引和ID，正文存KV 数据库；热数据在内存、冷数据在SSD
     - 缓存层：做语义缓存
